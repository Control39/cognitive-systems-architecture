...
def get_user_skills(user_id):
    ...
    return jsonify(
        [
            {"id": skill.id, "name": skill.name, "level": skill.level}
...
def update_user_skill(user_id, skill_id):
    ...
    if "name" in data:
        ...
        existing_skill = Skill.query.filter(
            Skill.name == data["name"], Skill.user_id == user_id, Skill.id != skill_id
        ).first()
        if existing_skill:
            return (
                jsonify({"error": "Skill with this name already exists for this user"}),
                400,
            )
