# 05 Documentation Docs Obsidian Map Scripts Compare Itcompass Versions

- **Путь**: `05_DOCUMENTATION\obsidian-map\05_DOCUMENTATION_docs_obsidian-map_scripts_compare_itcompass_versions.md`
- **Тип**: .MD
- **Размер**: 882 байт
- **Последнее изменение**: 2026-03-13 21:05:04

## Превью

```
# Scripts Compare Itcompass Versions

- **Путь**: `05_DOCUMENTATION\docs\obsidian-map\scripts_compare_itcompass_versions.md`
- **Тип**: .MD
- **Размер**: 795 байт
- **Последнее изменение**: 2026-03-12 11:24:56

## Превью

```
# Compare Itcompass Versions

- **Путь**: `scripts\compare_itcompass_versions.ps1`
- **Тип**: .PS1
- **Размер**: 6,282 байт
- **Последнее изменение**: 2026-03-10 19:02:48

## Превью

```
# compare_itcompass_versions.ps1
# Compare all IT-Compass versions in the system

$path
... (файл продолжается)
```

