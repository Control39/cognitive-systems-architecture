# 05 Documentation Docs Obsidian Map Scripts Generate Obsidian Map

- **Путь**: `05_DOCUMENTATION\obsidian-map\05_DOCUMENTATION_docs_obsidian-map_scripts_generate_obsidian_map.md`
- **Тип**: .MD
- **Размер**: 924 байт
- **Последнее изменение**: 2026-03-13 21:05:04

## Превью

```
# Scripts Generate Obsidian Map

- **Путь**: `05_DOCUMENTATION\docs\obsidian-map\scripts_generate_obsidian_map.md`
- **Тип**: .MD
- **Размер**: 880 байт
- **Последнее изменение**: 2026-03-12 11:24:56

## Превью

```
# Generate Obsidian Map

- **Путь**: `scripts\generate_obsidian_map.py`
- **Тип**: .PY
- **Размер**: 5,105 байт
- **Последнее изменение**: 2026-03-12 10:52:00

## Превью

```
"""
Генерирует Obsidian-карту знаний из структуры репозитория portfolio-system-architect.
"""

from datetime 
... (файл продолжается)
```

