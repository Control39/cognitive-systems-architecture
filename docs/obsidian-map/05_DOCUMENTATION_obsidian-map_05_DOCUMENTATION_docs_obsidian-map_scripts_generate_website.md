# 05 Documentation Docs Obsidian Map Scripts Generate Website

- **Путь**: `05_DOCUMENTATION\obsidian-map\05_DOCUMENTATION_docs_obsidian-map_scripts_generate_website.md`
- **Тип**: .MD
- **Размер**: 930 байт
- **Последнее изменение**: 2026-03-13 21:05:04

## Превью

```
# Scripts Generate Website

- **Путь**: `05_DOCUMENTATION\docs\obsidian-map\scripts_generate_website.md`
- **Тип**: .MD
- **Размер**: 864 байт
- **Последнее изменение**: 2026-03-12 11:24:56

## Превью

```
# Generate Website

- **Путь**: `scripts\generate_website.py`
- **Тип**: .PY
- **Размер**: 7,687 байт
- **Последнее изменение**: 2026-03-12 09:09:22

## Превью

```
"""
Генерирует профессиональный сайт-портфолио с поддержкой Mermaid и Git-статуса.
"""

import subprocess
from pathlib import Pat
... (файл продолжается)
```


