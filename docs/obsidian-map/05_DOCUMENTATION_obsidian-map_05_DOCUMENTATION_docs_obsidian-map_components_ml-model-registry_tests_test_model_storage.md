# 05 Documentation Docs Obsidian Map Components Ml Model Registry Tests Test Model Storage

- **Путь**: `05_DOCUMENTATION\obsidian-map\05_DOCUMENTATION_docs_obsidian-map_components_ml-model-registry_tests_test_model_storage.md`
- **Тип**: .MD
- **Размер**: 961 байт
- **Последнее изменение**: 2026-03-13 21:05:01

## Превью

```
# Components Ml Model Registry Tests Test Model Storage

- **Путь**: `05_DOCUMENTATION\docs\obsidian-map\components_ml-model-registry_tests_test_model_storage.md`
- **Тип**: .MD
- **Размер**: 870 байт
- **Последнее изменение**: 2026-03-12 11:25:17

## Превью

```
# Test Model Storage

- **Путь**: `components\ml-model-registry\tests\test_model_storage.py`
- **Тип**: .PY
- **Размер**: 2,816 байт
- **Последнее изменение**: 2026-03-08 16:11:38

## Превью

```
import unittest
import os
import tempfil
... (файл продолжается)
```


