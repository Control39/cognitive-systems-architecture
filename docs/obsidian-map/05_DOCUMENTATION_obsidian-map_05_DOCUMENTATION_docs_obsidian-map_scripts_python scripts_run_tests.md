# 05 Documentation Docs Obsidian Map Scripts Python Scripts Run Tests

- **Путь**: `05_DOCUMENTATION\obsidian-map\05_DOCUMENTATION_docs_obsidian-map_scripts_python scripts_run_tests.md`
- **Тип**: .MD
- **Размер**: 836 байт
- **Последнее изменение**: 2026-03-13 21:05:04

## Превью

```
# Scripts Python Scripts Run Tests

- **Путь**: `05_DOCUMENTATION\docs\obsidian-map\scripts_python scripts_run_tests.md`
- **Тип**: .MD
- **Размер**: 556 байт
- **Последнее изменение**: 2026-03-12 11:24:56

## Превью

```
# Run Tests

- **Путь**: `scripts\python scripts\run_tests.sh`
- **Тип**: .SH
- **Размер**: 334 байт
- **Последнее изменение**: 2026-03-10 19:02:48

## Превью

```
#!/bin/bash
# Получаем директорию тестов из конфигурации
TEST_DIR=$(python -c "import yaml; c=yaml.safe_load(open(
... (файл продолжается)
```


